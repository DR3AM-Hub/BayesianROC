...
BayesianPrior = (0.2, 0.2)  # None uses prevalence as a prior (population or sample as determined by previous item)

def testBayesianROC(descr, scores, labels, groupAxis='FPR', groups=[[0.0, 1/3], [1/3, 2/3], [2/3, 1.0]],
                    costs=dict(cFP=1, cFN=1, cTP=0, cTN=0, costsAreRates=False), BayesianPrior=None,
                    popPrevalence=None, showPlot = True):
    ...
    from Helpers.BayesianROCFunctions import resolvePrior
    ...
    ######################
    print('\naBayesianROC:')
    aBayesianROC  = BayesianROC(predicted_scores=scores, labels=labels, poslabel=poslabel,
                                BayesianPrior=BayesianPrior, costs=costs, quiet=quiet)

    if showPlot:
        print('\nROC plot: ', end='')
        print('plot shown.')
        aBayesianROC.plot(plotTitle=f'Full ROC Plot for Test {descr}',
                          showThresholds=True, showOptimalROCpoints=True, costs=costs,
                          saveFileName=None, numShowThresh=30, showPlot=showPlot)
    #endif

    if showPlot:
        print('\nConcordanceMatrixPlot(aBayesianROC):')
        aCMplot = ConcordanceMatrixPlot(aBayesianROC)
        print('Plot shown.')
        aCMplot.plot(plotTitle=f'Concordance Matrix for Test {descr}',
                     showThresholds=True, showOptimalROCpoints=True, costs=costs,
                     saveFileName=None, numShowThresholds=30, showPlot=showPlot, labelThresh=True)
    #endif

    print(f'\naBayesianROC.setGroupsBy: {groupAxis} = ', end='')
    printGroups(groups)
    aBayesianROC.setGroupsBy(groupAxis=groupAxis, groups=groups, groupByClosestInstance=False)
    numgroups = len(groups)

    # used in several sections that follow plotGroup, analyzeGroupVsChance
    prevalence = resolvePrevalence(popPrevalence, aBayesianROC.newlabels)
    prior      = resolvePrior(BayesianPrior, prevalence)
    aBayesianROC.prevalenceToPlot = prevalence
    aBayesianROC.priorToPlot      = prior

    if showPlot:
        print('\naBayesianROC.plotGroup:')
        aCMplot2 = ConcordanceMatrixPlot(aBayesianROC)
        for i in range(0, numgroups):
            print(f'ROC plot shown for group {i} [{groups[i][0]:0.2f}, {groups[i][1]:0.2f}]')
            aBayesianROC.plotGroup(plotTitle=f'Deep ROC Plot for group {i+1}, Test {descr}', groupIndex=i,
                                   showError=True, showThresholds=True, showOptimalROCpoints=True, costs=costs,
                                   saveFileName=None, numShowThresh=30, showPlot=True, labelThresh=True,
                                   full_fpr_tpr=True)
    ...
    print('\naBayesianROC.analyzeGroup:')
    aBayesianROC.analyze()

    print('\naBayesianROC.analyzeGroupVsPrior:')
    cpAUCi_pi_sum = 0
    for i in range(0, numgroups):
        measures_dict = aBayesianROC.analyzeGroupVsPrior(i, prevalence, costs, prior)
        if i == 0:
        ...
        showBayesianAUCmeasures(i+1, measures_dict, aBayesianROC.groupsArePerfectCoveringSet)
        if aBayesianROC.groupsArePerfectCoveringSet:
...
#endif
testBayesianROC(testNum, scores, labels, groupAxis=groupAxis, groups=groups, costs=costs, BayesianPrior=BayesianPrior,
